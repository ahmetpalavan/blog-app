...
export const RichTextComponents = {
  ...
  blocks: {
    ...
    blockquote: ({ children }: any) => <blockquote className="border-l-4 border-[#f7ab0a] pl-5 py-5 my-5">{children}</blockquote>,
  },
  marks: {
    link: ({ children, value }: any) => {
      const rel = !value.href.startsWith("/") ? "noopener noreferrer" : undefined;
      return (
        <Link className="underline decoration-[#f7ab0a] hover:decoration-black" rel={rel} href={value.href}>
          {children}
        </Link>
      );
    },
